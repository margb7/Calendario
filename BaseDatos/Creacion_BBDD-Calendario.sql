...
CREATE TABLE USUARIOS(
    ...
    PASSWD VARCHAR(30) NOT NULL,	-- TODO: revisar contrasinal
...
CREATE TABLE GRUPAIS_USUARIOS(
    ...
    INDEX FK_PARTICIPANTE(USUARIO),
    
    PRIMARY KEY (EVENTO_GRUPAL, USUARIO)
...
DROP VIEW IF EXISTS VISTA_EVENTOS_PUBLICOS;
CREATE VIEW VISTA_EVENTOS_PUBLICOS 
    AS
    SELECT E.ID_EVENTO AS ID_EVENTO,
            E.NOME AS NOME,
            E.DATA_EVENTO AS DATA_EVENTO,
            E.HORA AS HORA,
            E.CREADOR AS CREADOR
        FROM EVENTOS AS E INNER JOIN EVENTOS_PUBLICOS AS EP
            ON E.ID_EVENTO = EP.EVENTO;
            
DROP VIEW IF EXISTS VISTA_EVENTOS_GRUPAIS;  ## Contén todas as participacións de usuarios nos eventos
CREATE VIEW VISTA_EVENTOS_GRUPAIS           ## grupais xunto co creador
    AS
    SELECT E.ID_EVENTO AS ID_EVENTO,
            E.NOME AS NOME,
            E.DATA_EVENTO AS DATA_EVENTO,
            E.HORA AS HORA,
            E.CREADOR AS CREADOR,
            GU.USUARIO AS USUARIO   -- Participante
        FROM EVENTOS AS E INNER JOIN EVENTOS_GRUPAIS AS EG
            ON E.ID_EVENTO = EG.EVENTO
                INNER JOIN GRUPAIS_USUARIOS AS GU
                    ON GU.EVENTO_GRUPAL = E.ID_EVENTO;
